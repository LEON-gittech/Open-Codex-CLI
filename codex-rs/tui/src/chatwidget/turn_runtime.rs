...
impl ChatWidget {
    ...
    pub(super) fn foreground_turn_running(&self) -> bool {
        self.turn_lifecycle.agent_turn_running && !self.task_backgrounded
    }

    pub(super) fn restore_foreground_turn_activity(&mut self) {
        if !self.turn_lifecycle.agent_turn_running || !self.task_backgrounded {
            return;
        }
    ...
    pub(super) fn should_accept_live_turn_output(&self, turn_id: &str) -> bool {
    ...
    pub(super) fn on_task_complete(
        ...
        // Flush non-collab background activities (backgrounded exec cells) into history.
    ...
    pub(super) fn on_plan_update(&mut self, update: UpdatePlanArgs) {
        ...
        self.last_plan_update_items = update.plan.clone();
        self.refresh_status_surfaces();
