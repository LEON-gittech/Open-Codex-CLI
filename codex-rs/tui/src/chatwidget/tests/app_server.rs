...
#[tokio::test]
async fn background_subagent_interrupted_stays_active_in_down_panel() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let sender_thread_id = ThreadId::new();
    let spawned_thread_id = ThreadId::new();
    chat.set_collab_agent_metadata(
        ...
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: AppServerThreadItem::CollabAgentToolCall {
                ...
                agents_states: HashMap::from([(
                    spawned_thread_id.to_string(),
                    AppServerCollabAgentState {
                        status: AppServerCollabAgentStatus::Interrupted,
    ...
    chat.handle_key_event(KeyEvent::new(KeyCode::Down, KeyModifiers::NONE));
    let rendered = render_bottom_popup(&chat, /*width*/ 96);
    assert!(rendered.contains("Robie [explorer]: Interrupted"));
}
