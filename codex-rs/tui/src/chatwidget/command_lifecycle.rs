...
impl ChatWidget {
    ...
    pub(super) fn on_exec_command_output_delta(&mut self, call_id: &str, delta: &str) {
        ...
        // Check background activities first: if the call belongs to a backgrounded exec cell,
        // route the delta there instead of the foreground active cell.
    ...
    pub(crate) fn handle_command_execution_completed_now(&mut self, item: ThreadItem) {
        enum ExecEndTarget {
            ...
            // The call id was found in a backgrounded exec activity.
            BackgroundTracked(usize),
        ...
        let end_target = match self.transcript.active_cell.as_ref() {
            Some(cell) => match cell.as_any().downcast_ref::<ExecCell>() {
                ...
                Some(_) | None => {
                    if let Some(bg_index) = self.background_activities.iter().position(|activity| {
                        activity
                            .cell
                            .as_any()
                            .downcast_ref::<ExecCell>()
                            .is_some_and(|exec_cell| {
                                exec_cell.iter_calls().any(|call| call.call_id == id)
                            })
                    }) {
                        ExecEndTarget::BackgroundTracked(bg_index)
                    } else {
        ...
        match end_target {
            ...
            ExecEndTarget::BackgroundTracked(bg_index) => {
                if let Some(activity) = self.background_activities.get_mut(bg_index)
                    && let Some(exec_cell) = activity.cell.as_any_mut().downcast_mut::<ExecCell>()
                {
                    ...
                    debug_assert!(completed, "background exec cell should contain {id}");
                }
                if let Some(activity) = self.background_activities.remove(bg_index) {
                    self.transcript.needs_final_message_separator = true;
                    self.app_event_tx
