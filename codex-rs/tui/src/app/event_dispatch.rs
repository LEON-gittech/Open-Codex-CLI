...
impl App {
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::ShowMemoryBrowser => match app_server.memory_list().await {
                Ok(response) => self.chat_widget.open_memory_browser(response),
                Err(err) => self
                    .chat_widget
                    .add_error_message(format!("Failed to load memories: {err}")),
            },
            AppEvent::OpenMemorySettings => {
                self.chat_widget.open_memories_popup();
            }
            AppEvent::ShowMemoryOverlayStatus => match app_server.memory_overlay_status().await {
                Ok(status) => self.chat_widget.open_memory_overlay_status(status),
                Err(err) => self
                    .chat_widget
