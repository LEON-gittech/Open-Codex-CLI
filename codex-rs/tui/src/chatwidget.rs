...
pub(crate) struct ChatWidget {
    ...
    /// When `true`, the agent turn is still running but the TUI has moved it to
    /// the background so the user can continue composing.
    ///
    /// The core turn remains running and cancellable; this only suppresses the bottom-pane running
    /// state so the current session can continue accepting foreground input.
...
fn background_elapsed_text(started_at: Instant) -> String {
    crate::status_indicator_widget::fmt_elapsed_compact(started_at.elapsed().as_secs())
}
...
impl ChatWidget {
    ...
    /// Replaces the cached metadata for a thread.
    fn update_collab_agent_metadata(&mut self, thread_id: ThreadId, metadata: AgentMetadata) {
    ...
    /// Open the `/btw` side-question view in the bottom pane.
    pub(crate) fn show_btw_view(
        &mut self,
        thread_id: codex_protocol::ThreadId,
        state: Arc<std::sync::Mutex<history_cell::BtwQuestionCellState>>,
        cwd: PathBuf,
    ) {
        self.bottom_pane.show_btw_view(thread_id, state, cwd);
    }

    /// Returns the collaboration mode to use for the current submission.
    pub(crate) fn current_submission_collaboration_mode(
        ...
        let mut mode = self.current_collaboration_mode.clone();
        if let Some(mask) = &self.active_collaboration_mask {
            mode = mode.apply_mask(mask);
        }
        if xhigh_effort_override {
            mode.settings.reasoning_effort = Some(ReasoningEffortConfig::XHigh);
        }
        Some(mode)
    }

    /// Open the memory browser popup with the loaded data.
    ...
    /// Open the memory overlay status view with the loaded data.
    pub(crate) fn open_memory_overlay_status(
        &mut self,
        status: codex_app_server_protocol::MemoryOverlayStatusResponse,
    ) {
        use crate::bottom_pane::MemoryBrowserView;
        let view = MemoryBrowserView::from_overlay_status(status, self.app_event_tx.clone());
        self.bottom_pane.show_view(Box::new(view));
        self.request_redraw();
    }

    /// Mark a background terminal as stopping (from the background tasks panel).
    ...
    /// Mark a background subagent as stopping.
    pub(crate) fn mark_background_subagent_stopping(
        &mut self,
    ...
    /// Whether the chat widget has pending or queued input.
    ...
    pub(crate) fn refresh_background_tasks_view_if_open(&mut self) {
    ...
    pub(crate) fn show_background_activity_summary(&mut self) -> bool {
        ...
        if self.background_activities.is_empty()
            ...
            && self.last_plan_update_items.is_empty()
        {
            return false;
        }
    ...
    fn background_tasks_view_params(&self) -> crate::bottom_pane::BackgroundTasksViewParams {
        ...
        for item in &self.last_plan_update_items {
            let status = match item.status {
                codex_protocol::plan_tool::StepStatus::Completed => "x".to_string(),
                codex_protocol::plan_tool::StepStatus::InProgress => "*".to_string(),
                codex_protocol::plan_tool::StepStatus::Pending => " ".to_string(),
            };
            params.tasks.push(PlanTaskItem {
                status,
        ...
        for activity in &self.background_activities {
            ...
            let detail: Vec<String> = lines.iter().skip(1).map(line_to_plain_string).collect();
        ...
        for process in &self.unified_exec_processes {
            let detail = process.recent_chunks.clone();
            params.terminals.push(BackgroundTaskItem {
                ...
                detail,
                status: Some("running".to_string()),
                output_lines: process.recent_chunks.clone(),
                stoppable: true,
            });
        }
        params
    ...
    /// Handle a service tier command from the bottom pane.
    pub(crate) fn handle_service_tier_command_dispatch(
        &mut self,
        command: crate::bottom_pane::slash_commands::ServiceTierCommand,
    ) {
        self.toggle_service_tier_from_ui(command);
    }

    /// Open the agent profiles popup.
    pub(crate) fn open_agent_profiles_popup(&mut self) {
        use crate::bottom_pane::SelectionViewParams;

        let profiles = vec![
            ("default", "General-purpose assistant"),
            ("explorer", "Read-only exploration and analysis"),
            ("worker", "Implementation-focused task execution"),
        ];
        let items: Vec<crate::bottom_pane::SelectionItem> = profiles
            .into_iter()
            .map(|(name, description)| crate::bottom_pane::SelectionItem {
                name: name.to_string(),
        ...
        let params = SelectionViewParams {
            view_id: Some("agent_profiles"),
            title: Some("Agent profiles".to_string()),
            subtitle: Some(
                "Profiles define stable capability boundaries for subagents.".to_string(),
            ),
            items,
            ..Default::default()
        };
    ...
    /// Whether the current turn should be considered backgrounded.
    pub(crate) fn should_mark_current_turn_backgrounded(&self) -> bool {
    ...
    /// Move the current active exec cell to background activities and mark the
    /// turn as backgrounded so the user can continue composing input.
    ///
    /// Returns `true` if the activity was backgrounded, `false` if conditions
    /// were not met (already backgrounded, no running turn, modal active, etc.).
    pub(crate) fn background_current_activity(&mut self) -> bool {
    ...
    /// Returns (running, total) counts for background tasks.
    pub(crate) fn background_task_counts(&self) -> (usize, usize) {
