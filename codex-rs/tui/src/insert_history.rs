...
///
/// Standard terminals support `DECSTBM` scroll regions and Reverse Index (`ESC M`), which let us
/// slide existing content down without redrawing it. Zellij drops or mishandles those sequences, so
/// `Zellij` mode falls back to emitting newlines at the bottom of the screen and writing lines at
/// absolute positions.
